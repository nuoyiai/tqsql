...
import tpsql.reflector.ClassInfo;
import tpsql.reflector.PropertyInfo;
import tpsql.reflector.PropertyType;
...
public class ReflectorUtil {
		...
		if(!classCache.containsKey(c)){
			...
			for(Method method : methods){
				...
				if(!filterPropertyMethods.contains(name) && name.length()>3){
					...
					}else if(is.equals("is") && method.getParameterTypes().length==0){
						if(method.getReturnType()!=null && method.getReturnType().getName().equals("boolean")){
							String property = name.substring(2,3).toLowerCase()+name.substring(3);
							if(!properies.containsKey(property)){
								PropertyInfo pi = new PropertyInfo();
								pi.setGetMethod(method);
								pi.setPropertyType(getPropertyType(method.getReturnType()));
	...
	public static Object newInstance(Class<?> c){
		if(c!=null)
		{
			...
			throw new RuntimeException("创建的实列类型为能为空");
		}
	}

	public static Object addProperty(Object obj,String propertyName,Object propertyValue){
		BeanMap beanMap = BeanMap.create(obj);
		BeanGenerator generator = new BeanGenerator();
		generator.setSuperclass(obj.getClass());
		generator.addProperty(propertyName,propertyValue.getClass());
		Object newObj = generator.create();
		BeanMap newBeanMap = BeanMap.create(newObj);
		for(Object key : beanMap.keySet()){
			newBeanMap.put(key,beanMap.get(key));
		}
		newBeanMap.put(propertyName,propertyValue);
		return newObj;
	}
	 ...
	 * @return
	 */
	public static Object newProxy(Map<String,Class<?>> propertyMap){
		try{
			BeanGenerator generator = new BeanGenerator();
			Set<String> keySet = propertyMap.keySet();
			for (Iterator<String> i = keySet.iterator(); i.hasNext();) {
	...
	public static Object getFieldValue(Object obj,String fieldName){
		if(obj!=null) {
			try {
				Field field = getFile(obj.getClass(),fieldName);
				if(field!=null) {
	...
	public static Field getFile(Class<?> c,String fieldName){
		Field field = null;
		try {
		...
		while(field == null && c.getSuperclass() != null){
			return getFile(c.getSuperclass(),fieldName);
		}
		return field;
	}
