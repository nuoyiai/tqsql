...
import tpsql.sql.builder.NullParameter;
...
public class SqlItemClause extends SqlItemContent implements ISqlItemClone<SqlItemClause>,ISqlParamMacro {
	...
	{
		if(StringUtil.isNotEmpty(cAssert))
		{
	...
	public SqlItemClause clone(Object args){
		...
		if(this.cAssert!=null) {
			clause.cAssert = this.cAssert.clone(args);
		}
		clause.parts=this.cloneParts(args);
		if(this.parent!=null) {
			clause.setParent(this.parent);
		}
		return clause;
	}
	...
	public String format(String text)
	{
		if(this.paramNames.length>0 && text.indexOf('{')>-1)
		{
			...
			if(hasParamChar) {
				text = this.formatParam(text, this.paramNames);
			}
			String str = StringUtil.format(text,(Object[])names);
			return str;
		}
		else {
			return text;
	...
	public SqlString toString(SqlItemArgs arg){
		...
		if(StringUtil.isNullOrEmpty(this.prepend)){
			...
			if(StringUtil.isNotEmpty(this.getNvl()) && part.isEmpty()){
			...
			if(arg.getHidePrepend()){
				...
				if(StringUtil.isNotEmpty(this.getNvl()) && part.isEmpty()){
					...
					return new SqlString(this.prepend + " ").contact(part);
				}
			}
		}
	}
	...
	class ClauseAssert implements ISqlItemClone<ClauseAssert>
	{
		...
		private void parserMehtod(String expression)
		{
			...
			int m = str.indexOf(")");
			if(n<0 || m<0) {
				throw new SqlMapException(StringUtil.format("解析断言表达式出错 {0}", expression));
			} else
			{
				if(str.charAt(0)=='!')
				{
		...
		public boolean isTrue(SqlItemArgs arg)
		{
			...
			if(ca!=null)
			{
				if(StringUtil.isNotEmpty(this.methodParams))
				{
					...
					for(int i=0;i<params.length;i++)
					{
						if(params[i]!=null)
						{
							...
							throw new SqlMapException(StringUtil.format("断言表达式参数 为空 {0}", this.methodParams));
						}
					}
					if(this.hasOppositeChar) {
						return !ca.invoke(arg.getDialect(), args);
					} else {
						return ca.invoke(arg.getDialect(), args);
					}
				}
				...
				{
					if(this.hasOppositeChar) {
						return !ca.invoke(arg.getDialect(), new Object[0]);
					} else {
						return ca.invoke(arg.getDialect(), new Object[0]);
					}
				}
				...
				throw new SqlMapException(StringUtil.format("找不到断言函数 {0}", this.methodName));
			}
		}
	}
}
