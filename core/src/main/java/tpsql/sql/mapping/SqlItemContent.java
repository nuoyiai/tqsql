	...
	class ContentTextWrapper implements ISqlItemPart,ISqlItemClone<ContentTextWrapper>
	{
		...
		public SqlString toString(SqlItemArgs arg) {
			//synchronized(this.lockObj){
				if(this.parts==null && StringUtil.isNotEmpty(this.content)){
		...
		private SqlString parserParam(ContentTextPart part,SqlItemArgs arg){
			switch(part.type){
				...
				case 1:{
					...
					boolean hasParamFlag = lastParam.charAt(1)=='$' || lastParam.charAt(1)=='#';
					if(value!=null){
						if(TypeUtil.isValue(value) || TypeUtil.isCollection(value)){
							if(hasParamFlag)return new SqlString(new Parameter(value));
							else{
								if(value instanceof String) {
									String paramValue = (String)value;
									if (arg.getIsSelect() && paramValue.indexOf("'") > -1) {			//放Sql注入
						...
						return hasParamFlag?new SqlString(new NullParameter()):null;
					}
				}
				case 2:{
					boolean hasParamFlag = part.text.charAt(1)=='$' || part.text.charAt(1)=='#';
					Object value = arg.parserParam(part.text);
					if(hasParamFlag){
						return new SqlString(new Parameter(value));
					}else {
