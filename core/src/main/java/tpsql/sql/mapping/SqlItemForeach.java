...
public class SqlItemForeach extends SqlItemContent implements ISqlItemClone<SqlItemForeach> {
	...
	public SqlString toString(SqlItemArgs arg) {
		if(StringUtil.isNotEmpty(inParamName)){
				...
				if(TypeUtil.isCollection(inParam)){
					...
					if(ps.size()>0){
						if(arg.getVariables()==null) {
							arg.setVariables(new HashMap<String, Object>());
						}
						...
						for(Object p : ps){
							...
							if(StringUtil.isNotEmpty(this.split) && i>0) {
					...
					throw new SqlMapException(StringUtil.format("循环表达式输入参数必须是数组或集合  {0}", inParamName));
				}
			}
			else {
				return null;
			...
			throw new SqlMapException("循环表达式输入参数名不能为空 ");
		}
	}
}
