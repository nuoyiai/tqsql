...
public class DefaultConfigFactory implements IDbConfigFactory {
    ...
    public void add(IDbConfig config){
        if(!this.configs.containsKey(config.getName())){
            ...
            if(defaultConfig==null || (defaultConfig!=null && !defaultConfig.isDefault() && config.isDefault())) {
    ...
    private List<String> getConfigBeanNames(){
        ...
        for(String name : names){
            if(!this.filterConfigs.contains(name)) {
				list.add(name);
			}
        }
        return list;
	...
	public IDbConfig getConfig() {
        ...
        if(config==null) {
			throw new SqlDriverException("找不到默认的数据库配置项");
		}
        return config;
    }
