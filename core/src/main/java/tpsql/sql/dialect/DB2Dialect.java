...
public class DB2Dialect extends AbstractDialect {
	...
	protected String removeTableAlias(String orderby){
		...
		while(m.find()){
			...
			int n = group.indexOf(".");
			if(n>-1) {
				result = result.replace(group, group.substring(n + 1));
			}
		}
		return result;
	}
	...
	class DB2Meta extends BaseSqlMeta
	{
		...
		public String getTablesSql() {
			return "select name as TABLE_NAME from sysibm.systables where type='T' order by name";
		}
		
		
		public String getTablesSql(String tableName) {
			return "select name as TABLE_NAME from sysibm.systables where type='T' and name='"+tableName+"' order by name";
		}
		...
		public String getFieldsSql(String tableName){
			return "select TABNAME AS TABLE_NAME,COLNAME AS FIELD_NAME,TYPENAME AS FIELD_TYPE,LENGTH AS FIELD_LENGTH,SCALE AS FIELD_SCALE,0 AS FIELD_PRECISION,REMARKS AS FIELD_REMARK from syscat.columns where tabname='"+tableName+"'";
		}
