...
import tpsql.core.collection.DataColumn;
...
import tpsql.sql.meta.AbstractDbMetaAccess;
...
public class SqlHelper implements ISqlHelper {
    ...
    public static IDataProviderBuilder getBuilder(){
        return (IDataProviderBuilder)SpringContext.instance().get(DataProviderBuilder.class);
    }
    ...
    public List<Object> executeScalars(String sqlString){
        this.setSqlCommand(sqlString);
        List<Object> objs = this.provider.getCommand().executeScalars().getResultList();
        this.provider.getConnection().close();
        return objs;
    }
    ...
    public List<Object> executeScalars(SqlString sqlString){
        this.setSqlCommand(sqlString);
        IDbCommand command = this.provider.getCommand();
        List<Object> objs = command.executeScalars().getResultList();
        sqlString.setExecTime(command.getExecTime());
        this.provider.getConnection().close();
        return objs;
    }
     ...
     * 执行Sql语句,返回引响行数
     */
    public SqlCommandResult executeNoneQuery(String sqlString){
        this.setSqlCommand(sqlString);
        SqlCommandResult result = this.provider.getCommand().executeNonQuery();
        this.provider.getConnection().close();
        return result;
    }
     ...
     * 批量执行Sql语句,返回引响行数
     */
    public SqlCommandResult executeNoneQuery(String[] sqlStrings){
     ...
     * 批量执行Sql语句,返回引响行数
     */
    public SqlCommandResult executeNoneQuery(String[] sqlStrings,boolean batchCommit){
        if(batchCommit) {
            this.setSqlCommand(sqlStrings);
            SqlCommandResult result = this.provider.getCommand().executeNonQuery();
            this.provider.getConnection().close();
            ...
            for(int i=0;i<sqlStrings.length;i++){
                this.setSqlCommand(sqlStrings[i]);
                obj[i] = this.provider.getCommand().executeNonQuery().getRowFirst();
            }
     ...
     * 执行Sql语句,返回引响行数
     */
    public SqlCommandResult executeNoneQuery(SqlString sqlString){
        this.setSqlCommand(sqlString);
        IDbCommand command = this.provider.getCommand();
        SqlCommandResult result = command.executeNonQuery();
        sqlString.setExecTime(command.getExecTime());
        return result;
    }
     ...
     * 执行Sql语句,返回引响行数和自增id
     */
    public SqlCommandResult executeNoneQuery(SqlString sqlString,boolean autoIncrementFlag){
        this.setSqlCommand(sqlString);
        IDbCommand command = this.provider.getCommand();
        SqlCommandResult result;
        if(autoIncrementFlag) {
            String autoIncrementSql = this.provider.getDialect().getMetaSql().getAutoIncrementSql();
            result = command.executeNonQuery(autoIncrementSql);
        }else{
     ...
     * 批量执行Sql语句,返回引响行数
     */
    public SqlCommandResult executeNoneQuery(SqlString[] sqlStrings){
        this.setSqlCommand(sqlStrings);
        IDbCommand command = this.provider.getCommand();
        SqlCommandResult result = command.executeNonQuery();
     ...
     * 批量执行Sql语句,返回引响行数
     */
    public SqlCommandResult executeNoneQuery(SqlString[] sqlStrings, boolean batchCommit){
        if(batchCommit) {
            this.setSqlCommand(sqlStrings);
            IDbCommand command = this.provider.getCommand();
            SqlCommandResult result = command.executeNonQuery();
    ...
    protected DataTable getDataTable(String tableName,IDataReader reader){
        ...
        List<Column> columnList = reader.getColumns();
        boolean logFlag = log.isDebugEnabled();
        String columnInfo = "[";
        String rowInfo = "";
        for(Column column : columnList){
            if(!table.contains(column.getLabel()))table.addColumn(column.getLabel(),column.getClassType());
            else throw new SqlExecuteException("返回数据集结果列 "+column.getLabel()+" 重名");
            if(logFlag)columnInfo+=(columnInfo.length()>1)?","+column.getLabel():column.getLabel();
        }
    ...
    private void setSqlCommand(SqlString sqlString){
        ...
        if(log.isInfoEnabled()){
            log.info(sql);
            if(sqlString.getParamValues()!=null && sqlString.getParamValues().length>0){
                String paramInfo = "[";
    ...
    protected List<?> getProxyListByDataTable(DataTable table){
        List list = new ArrayList();
        if(table!=null && table.rowSize()>0 && table.colSize()>0){
            Map<String,Class<?>> propertyMap = new HashMap<String, Class<?>>();
            for(DataColumn column : table.getColumns()){
                String propertyName = StringUtil.getLowerName(column.getName());
                if(column.getType().getName().equals("oracle.sql.TIMESTAMP")) {
                    propertyMap.put(propertyName, java.sql.Timestamp.class);
                }else{
                    propertyMap.put(propertyName, column.getType());
                }
            }
            for(int i=0;i<table.rowSize();i++) {
                ...
                for(DataColumn column : table.getColumns()) {
                    String propertyName = StringUtil.getLowerName(column.getName());
                    ReflectorUtil.setPropertyValue(proxy,propertyName,column.get(i));
                }
                list.add(proxy);
            }
        }
     ...
     * @return
     */
    public List<Table> getMetaTableList(){
        try {
            this.getDriver().getConnection().open();
            Connection conn = this.getDriver().getConnection().getConnection();
            DatabaseMetaData dbMetaData = conn.getMetaData();
            String schema = (String)ReflectorUtil.getFieldValue(dbMetaData,"database");
            String tableSqlString = this.getDriver().getDialect().getMetaSql().getTablesSql();
            IDataReader reader = this.getDataReader(tableSqlString);
            Map<String,Table> tableMap = new HashMap<String,Table>();
            List<Table> tableList = new ArrayList<Table>();
            while(reader.read()){
                String tableSchema = Reader.stringReader().getValue(reader,"TABLE_SCHEMA");
                if(StringUtil.isNullOrEmpty(tableSchema) || schema.equals(tableSchema)){
                    String tableName = Reader.stringReader().getValue(reader,"TABLE_NAME");
                    String tableRemark = Reader.stringReader().getValue(reader,"TABLE_REMARK");
                    ...
                    ResultSet rs = dbMetaData.getPrimaryKeys(schema, schema, tableName);
                    String pkStr = "";
                    while (rs.next()) {
                        String pkColumnName = rs.getString("COLUMN_NAME");
                        pkStr+="".equals(pkStr)?pkColumnName:","+pkColumnName;
                    }
                    table.setPkName(pkStr);
                    rs.close();
                }
            }

            ResultSet rs = dbMetaData.getColumns(schema, schema,"%", "%");
            while(rs.next()){
                String tableName = rs.getString("TABLE_NAME");
                if(tableMap.containsKey(tableName)) {
                    Table table = tableMap.get(tableName);
                    Column column = getColumn(rs);
                    column.setPrimaryKey(table.getPkName().contains(column.getName()));
     ...
     * @return
     */
    public Table getMetaTable(String tableName){
        try {
            Table table = new Table();
            table.setName(tableName);
            this.getDriver().getConnection().open();
            ...
            DatabaseMetaData dbMetaData = conn.getMetaData();
            String schema = (String)ReflectorUtil.getFieldValue(dbMetaData,"database");
            String tableSqlString = this.getDriver().getDialect().getMetaSql().getTablesSql(tableName,schema);
            String tableRemark = Reader.stringReader().getValue(this.getDataReader(tableSqlString),"TABLE_REMARK");
            table.setRemark(tableRemark);
            ResultSet rs = dbMetaData.getPrimaryKeys(schema, schema, tableName);
            String pkStr = "";
            ...
            rs.close();
            table.setPkName(pkStr);
            rs = dbMetaData.getColumns(schema, schema,tableName, "%");
            List<Column> columnList = new ArrayList<Column>();
            while(rs.next()){
                Column column = getColumn(rs);
                column.setPrimaryKey(pkSet.contains(column.getName()));
    ...
    private Column getColumn(ResultSet rs) throws SQLException {
        ...
        int columnLength = rs.getInt("COLUMN_SIZE");  //列大小
        int scale = rs.getInt("DECIMAL_DIGITS");  //小数位数
        int precision = rs.getInt("NUM_PREC_RADIX");
        int nullable = rs.getInt("NULLABLE");  //是否允许为null
        Column column = new Column();
        column.setName(columnName);
        column.setComment(remarks);
        column.setTypeName(dbType);
    ...
    public static boolean isSelectSqlString(String sqlString){
        ...
        if(m.find()){
            String str = m.group();
            if(StringUtil.isNotEmpty(str) && "select".equals(str.toLowerCase())){
                return true;
            }
        }
        return false;
     ...
     * @return
     */
    public synchronized Map<String,Object> getMetaDataMap(){
        if(metaDataMap==null) {
            ...
            try {
                ...
                Connection conn = this.getDriver().getConnection().getConnection();
                DatabaseMetaData dbMetaData = conn.getMetaData();
                String database = (String)ReflectorUtil.getFieldValue(dbMetaData,"database");
                metaDataMap.put("database",database);
                metaDataMap.put("schema",database);
            } catch (SQLException e) {
                e.printStackTrace();
            }
        }
