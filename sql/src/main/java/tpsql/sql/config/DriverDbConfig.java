	...
	public Connection getConnection() throws SQLException {
		if(this.dataSource!=null) {
			...
			if(StringUtil.isNullOrEmpty(this.url)) {
				throw new SqlDriverException("数据库连接配置 url属性不能为空");
			}
			if(StringUtil.isNullOrEmpty(this.getDriverClass())) {
				throw new SqlDriverException("数据库连接配置 driverClass为空");
			}
			try {
				Driver driver = null;
				if(this.driver==null){
					Class<?> driverClass = Class.forName(this.getDriverClass());
					if(driverClass==null){
						URLClassLoader classLoader = new URLClassLoader(new URL[]{new URL("file:/"+this.getJar())});
						driverClass = classLoader.loadClass(this.getDriverClass());
					}
					if(driverClass==null) {
						throw new SqlDriverException("找不jdbc驱动类:" + this.getDriverClass());
					}
