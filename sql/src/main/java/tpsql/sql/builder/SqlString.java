...
public class SqlString {
    	...
    	if(sqlParts!=null){
	    	for(Object obj : sqlParts){
	    		if(obj instanceof String){
	    			...
	    			if(((Parameter)obj).getValue()!=null) {
						return false;
					}
	    		}else if(obj!=null){
	    			return false;
    ...
    {
    	this(StringUtil.isNotEmpty(sqlPart)?new Object[] { sqlPart }:new Object[0]);
    }
	...
	public boolean isSelect(){
		return compact().toString().toLowerCase().indexOf("select")>-1;
	}
    	...
    	if(isCompact){
    		...
    		for(int i=0;i<sqlParts.length;i++)
    		{
    			...
    			if(part instanceof String)
    			{
    				...
    				if(StringUtil.isNotEmpty(strPart))
    				{
    					if(currentPart==null) {
							currentPart = strPart;
						} else {
